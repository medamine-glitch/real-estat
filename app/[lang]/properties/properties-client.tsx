...
import { Bed, Bath, Maximize, MapPin, Search, ArrowUpRight, X, Filter, RotateCcw } from "lucide-react"
...
export default function PropertiesClient({ dict, lang, properties }: PropertiesClientProps) {
  const {
    filters,
    filteredProperties,
    updateFilter,
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold">{dict.properties.filter.title}</h2>
              ...
              <div className={`space-y-6 ${showFilters ? "block" : "hidden lg:block"}`}>
                ...
                >
                  <Label htmlFor="location">{dict.properties.filter.location}</Label>
                  <Select value={filters.location} onValueChange={(value) => updateFilter("location", value)}>
                    <SelectTrigger id="location">
                      <SelectValue placeholder={dict.properties.filter.location} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                >
                  ...
                  <Select value={filters.type} onValueChange={(value) => updateFilter("type", value)}>
                    ...
                    <SelectContent>
                      ...
                      {Object.entries(dict.properties.types).map(([key, value]) => (
                        <SelectItem key={key} value={key}>
                          {value as string}
                        </SelectItem>
                      ))}
                ...
                >
                  <Label htmlFor="bedrooms">{dict.properties.filter.bedrooms}</Label>
                ...
                >
                  <div className="flex gap-2">
                    <Button className="flex-1" onClick={applyFilters} disabled={isLoading || !validation.isValid}>
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {filters.location && filters.location !== "all" && (
                        <Badge variant="secondary" className="flex items-center gap-1">
                          {dict.home.locations[filters.location as keyof typeof dict.home.locations]}
                          <X className="h-3 w-3 cursor-pointer" onClick={() => updateFilter("location", "all")} />
                        </Badge>
                      )}
                      {filters.type && filters.type !== "all" && (
                        <Badge variant="secondary" className="flex items-center gap-1">
                          {dict.properties.types[filters.type as keyof typeof dict.properties.types]}
                      ...
                      {filters.bedrooms && filters.bedrooms !== "any" && (
                        <Badge variant="secondary" className="flex items-center gap-1">
                          ...
                          <X className="h-3 w-3 cursor-pointer" onClick={() => updateFilter("bedrooms", "any")} />
                        </Badge>
                      )}
                    </div>
                  </motion.div>
        ...
        <div className="lg:col-span-3">
          <AnimatedElement animation="fadeInRight" className="flex justify-between items-center mb-6">
            <div>
            ...
            <Select value={sortBy} onValueChange={setSortBy}>
              ...
              <SelectContent>
                <SelectItem value="newest">{dict.properties.sort.newest}</SelectItem>
                <SelectItem value="priceAsc">{dict.properties.sort.priceAsc}</SelectItem>
                <SelectItem value="priceDesc">{dict.properties.sort.priceDesc}</SelectItem>
              </SelectContent>
            </Select>
          </AnimatedElement>
          ...
          {filteredCount === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {sortedProperties.map((property, index) => (
                <AnimatedElement key={property.id} animation="fadeInUp" delay={0.1 * index} className="h-full">
                  <motion.div className="h-full" whileHover={{ y: -5 }} transition={{ type: "spring", stiffness: 300 }}>
                    <Card className="overflow-hidden bg-card hover:shadow-lg transition-shadow h-full flex flex-col">
                      <Link href={`/${lang}/properties/${property.id}`} className="relative h-48 block">
                        <Image
                      ...
                      <CardContent className="p-4 flex flex-col flex-grow">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className="text-primary font-bold text-sm">{formatPrice(property.price, lang)}</span>
                        </div>
